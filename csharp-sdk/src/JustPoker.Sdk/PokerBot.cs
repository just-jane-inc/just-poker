...
public sealed class PokerBot : IAsyncDisposable
{
    ...
    private GamePlayerDTO? _player;
    private GameGameDTO? _currentState;
    ...
    public GameGameDTO? CurrentState => _currentState;
    public GamePlayerDTO? Player => _player;
    public IGameApi GameApi => _gameApi;
    public IUserApi UserApi => _userApi;
    public IReadOnlyList<Chips> CurrentStack => _currentStack;
    
    public IReadOnlyList<Card> HoleCards => _player?.Hole?.Select(Card.FromDto).ToList() ?? [];
    

    /// <summary>
    /// A poker bot
    /// </summary>
    /// <param name="baseUrl"></param> The url of the game server to connect to
    /// <param name="token"></param> the user authorization token to use for this bot
    /// <param name="userId"></param> the user id of the requested user for this bot
    /// <param name="gameId"></param> the id of the game to interact with
    /// <param name="timeout"></param> the timeout time (as timespan) the bot should wait for its turn when performing an action. Null or 0 to disable.
        ...
        TimeSpan? timeout = null, ILogger? logger = null)
    {
        ...
        {
            throw new PokerException("user id not provided");
        }
        ...
        _gameApi = _provider.GetRequiredService<IGameApi>();
        _userApi = _provider.GetRequiredService<IUserApi>();
    }
    ...
    public PokerEventHub Events
    {
        get
        {
            ...
            if (_stateSubscription is not null)
            {
    ...
    {
        int sum = 0;
        foreach (var chip in CurrentStack)
        {
            sum += chip.Value;
        }
        return sum;
    ...
    public Task<PokerEventHub> StartEventsAsync() => Events.StartAsync();
    ...
    public Task StopEventsAsync()
    {
        if (_hub is null)
        {
            throw new PokerException("no event hub has been started");
        }
    ...
    /// </summary>
    /// <param name="give"></param> the chips the bot is giving to the server exchange
    /// <param name="receive"></param> the chips the bot wants to receive as a result of the exchange
    /// <exception cref="PokerException"></exception> Raises if the exchange is determined invalid by the server
    public async Task ExchangeChipsAsync(IEnumerable<Chips> give, IEnumerable<Chips> receive)
    {
        ...
        foreach (var chip in give)
        {
            giveStack[chip.Denomination.ToString()] = chip.Count;
        }
        foreach (var chip in receive)
        {
            receiveStack[chip.Denomination.ToString()] = chip.Count;
        }
    ...
    {
        _logger.LogDebug("try cover bet for [{Amount}] stack: [{Stack}]", amountNeeded, string.Join(" ", _currentStack));
    ...
    private async Task<List<Chips>> TryCoverBetAsync(int amountNeeded, List<Chips> bet)
    {
        ...
        var denominations = _currentState?.GameConfig?.ChipDenominations;
        ...
        foreach (var chips in _currentStack.OrderByDescending(chips => chips.Denomination))
        {
            ...
            foreach (var brokenChips in broken)
            {
                MergeStack(brokenChips);
            }
            return await TryCoverBetAsync(amountNeeded, bet);
        }

        throw new InvalidBetException($"could not construct a bet covering {amountNeeded} from [{string.Join(" ", _currentStack)}]");
    }
    ...
    public async Task<bool> RaiseAsync(int raiseTo)
    {
        ...
        var currentBet = PokerHelpers.ChipSum(_player.CurrentBet ?? []);
        raiseTo -= currentBet;
        List<Chips> bet = await TryCoverBetAsync(raiseTo);
    ...
    /// <exception cref="PokerException"></exception> Raises if the bot is not in a joined game
    public async Task WaitForMyTurnAsync(CancellationToken cancellationToken = default)
    {
        if (!_joined) throw new PokerException("you are not in the game - it can never be your turn");
    ...
    private void IngestGameDto(GameGameDTO? state)
    {
        ...
        foreach (var gamePlayerDto in state.Table?.Players ?? [])
        {
            ...
            {
                _joined = true;
                _player =  gamePlayerDto;
                _currentStack = PokerHelpers.ConvertStack(_player?.Stack ?? new Dictionary<string, int>());
                break;
            }
        }
        _currentState = state;
    ...
    {
        if (pokerEvent.DataAs<GameGameDTO>() is { } state)
        {
            IngestGameDto(state);
        }
